...
#
#             true_label = data[qid_doc_map[qid], 0]
#             num_positive = int(np.sum(true_label > 0))
#
#             # Convert to binary labels
#             binary_labels = np.zeros(len(true_label))
#             top_indices = np.argsort(sub_pred_score)[::-1][:num_positive]
#             binary_labels[top_indices] = 1
#
#             all_true_labels.extend(binary_labels)
#             all_pred_scores.extend(sub_pred_score)
#
#             index_label = {}
#             index_score = {}
#             for i, s in zip(subset, sub_pred_score):
...
#
#         # 打印用于绘制ROC曲线的数值
#         print("False Positive Rate (fpr) for ROC curve:", fpr)
#         print("True Positive Rate (tpr) for ROC curve:", tpr)
#         print("Area Under the Curve (AUC) for ROC curve:", roc_auc)
#
#         # 打印用于绘制PR曲线的数值
#         print("Precision for PR curve:", precision)
#         print("Recall for PR curve:", recall)
#         # Plot ROC curve
#         plt.figure()
...
#             mrr = 0
#
#         mymetric = [hit10, hit3, hit1, mrr]
#         return np.nanmean(ndcg_list), roc_auc, mymetric
...
#         valid_queries = 0
#
#         # 存储每个疾病的前 10 个预测药物的特征
#         top_10_drug_features_per_disease = {}
#
#         file = open(file, 'w', encoding='utf8')
#         for qid in qid_doc_map.keys():
#             subset = qid_doc_map[qid]
...
#
#             rank = sorted(index_score.items(), key=lambda item: item[1], reverse=True)
#             top_10_indices = list(dict(rank).keys())[:10]
#             top_10_features = [tuple(data[idx, 2:]) for idx in top_10_indices]
#             top_10_drug_features_per_disease[qid] = set(top_10_features)
#
#             lst = []
#             for e in top_10_indices:
#                 lst.append(str(e))
#             file.write(str(int(qid)) + "\t" + ",".join(lst) + "\n")
#
#             h1 = 0
...
#         file.close()
#
#         # 计算不同疾病的前 10 个预测药物的重叠比例
#         overlap_ratios = []
#         qids = list(top_10_drug_features_per_disease.keys())
#         num_diseases = len(qids)
#         for i in range(num_diseases):
#             for j in range(i + 1, num_diseases):
#                 features_i = top_10_drug_features_per_disease[qids[i]]
#                 features_j = top_10_drug_features_per_disease[qids[j]]
#                 overlap = len(features_i.intersection(features_j)) / 10
...
#         # print("Recall for PR curve:", recall)
#
#         # 打印不同疾病的前 10 个预测药物的平均重叠比例
#         print("Average overlap ratio of top 10 predicted drugs for different diseases:", average_overlap_ratio)
#
#         # Plot ROC curve
#         plt.figure()
#         plt.plot(fpr, tpr, color='darkorange', lw=2, label='ROC curve (area = %0.2f)' % roc_auc)
...
#
#         mymetric = [hit10, hit3, hit1, mrr]
#         return np.nanmean(ndcg_list), roc_auc, mymetric, average_overlap_ratio
...
class LambdaRank:
    ...
    def validate(self, output_file, drug_rank_file, data, k):
        """
        validate the NDCG metric
        :param data: given the testset
        :param k: used to compute the NDCG@k
        ...
        # 存储每个疾病的前 20 个预测药物的特征
        top_20_drug_features_per_disease = {}

        output_file = open(output_file, 'w', encoding='utf8')
        drug_rank_file = open(drug_rank_file, 'w', encoding='utf8')
        ...
        for qid in qid_doc_map.keys():
            ...
            rank = sorted(index_score.items(), key=lambda item: item[1], reverse=True)
            top_20_indices = list(dict(rank).keys())[:5]
            top_20_features = [tuple(data[idx, 2:]) for idx in top_20_indices]
            top_20_drug_features_per_disease[qid] = set(top_20_features)

            # 输出每个疾病对应的所有药物按打分从高到低的结果
            drug_rank_file.write(f"disease_id: {qid}\n")
            for idx, score in rank:
                drug_rank_file.write(f"drug_index: {idx}, score: {score}\n")
            drug_rank_file.write("\n")

            lst = []
            for e in top_20_indices:
                lst.append(str(e))
            output_file.write(str(int(qid)) + "\t" + ",".join(lst) + "\n")
        ...
        output_file.close()
        drug_rank_file.close()

        # 计算不同疾病的前 20 个预测药物的重叠比例
        overlap_ratios = []
        qids = list(top_20_drug_features_per_disease.keys())
        num_diseases = len(qids)
        for i in range(num_diseases):
            for j in range(i + 1, num_diseases):
                features_i = top_20_drug_features_per_disease[qids[i]]
                features_j = top_20_drug_features_per_disease[qids[j]]
                overlap = len(features_i.intersection(features_j)) / 5
                overlap_ratios.append(overlap)
        ...
        # 打印不同疾病的前 20 个预测药物的平均重叠比例
        print("Average overlap ratio of top 20 predicted drugs for different diseases:", average_overlap_ratio)
