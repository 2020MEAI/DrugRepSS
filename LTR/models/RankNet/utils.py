...
def eval_ndcg_at_k(
        inference_model, device, df_train, valid_loader, batch_size, k_list, epoch,
    ...
    hit1=0
    mrr=0
    example_file=open("./resultdata/example_rankNet_1v1.txt",'w',encoding='utf8')
    # print("Eval Phase evaluate NDCG @ {}".format(k_list))
    ndcg_metrics = {k: NDCG(k) for k in k_list}
    qids, rels, scores = [], [], []
    # inference_model.eval()
    with torch.no_grad():
        for qid, rel, x in valid_loader.generate_query_batch(df_train, batch_size):
