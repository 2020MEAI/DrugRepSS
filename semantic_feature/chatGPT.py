...
# 替换为 OpenAI API 密钥
    api_key = "sk-xfph3AdPh6Mlo2yJPRAspuje0oY8m1A72WEMeGY8zL4nNHeN"
...
def readDrugName():
    ...
    id = []
    # 指定 Excel 文件的路径
    excel_file = "D:\yanjiu\GCL+BPR\Cdatasets\DrugName_id.xlsx"
...
def readDiseaseName():
    disease_id = {}
    # 指定 Excel 文件的路径
    excel_file = "D:\yanjiu\GCL+BPR\Cdatasets\DiseaseName_id.xlsx"
    # 使用 pandas 读取 Excel 文件
    df = pd.read_excel(excel_file)
    # 遍历 Excel 表格的每一行，将第二列数据作为键，第一列数据作为值存入字典
    for index, row in df.iterrows():
...
df = pd.read_excel("D:\yanjiu\GCL+BPR\Cdatasets\DrugName_id.xlsx")
...
df.to_excel("D:\yanjiu\GCL+BPR\Cdatasets\drug_id_vector1.xlsx", index = False)
print(f"药物向量在{datetime.datetime.now()}生成完毕")

# df = pd.read_excel("D:\yanjiu\GCL+BPR\Cdatasets\DiseaseName_id.xlsx")
# disease_em = {}
...
# df.to_excel("D:\yanjiu\GCL+BPR\Cdatasets\disease_id_vector.xlsx", index = False)
